...
class DPD_service
{
	...
	public function __construct()
	{
		...
		$this->wsdlclient = new SoapClient($this->arDPD_HOST[0] . "calculator2?wsdl");
	 ...
	 * @return
	 */
	public function getCityList($asis=false)
	{
	...
	//	}
		...
		try {
			...
			if ($client) {$obj = $client->getServiceCost2($arRequest);}
			else{
				$err->error="Ошибка поподключения к ". $this->arDPD_HOST[0] . "calculator2?wsdl";
return $err;
			}
		
		} catch (SoapFault $e) {
		
		$err->error="Ошибка получения данных dpdcityid=\"" . $arData['delivery']['cityId'] . "\" вес=".$arData['weight']."$e</br>";
return $err;
		}
